<?php
// admin/voucher-deposit.php
require_once('../common/config.php');
include(__DIR__ . "/includes/header.php");

// Fetch deposit heads
$heads = $pdo->query("SELECT id, name FROM voucher_heads WHERE type = 'deposit' AND status = 1 ORDER BY name ASC")->fetchAll(PDO::FETCH_ASSOC);
...
// Fetch existing deposit transactions
$stmt = $pdo->prepare("
...
<div class="pagetitle">
    <h1>Income / Deposit Vouchers</h1>
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?php echo ADMIN_BASE_URL; ?>index.php">Home</a></li>
            <li class="breadcrumb-item">Office Accounting</li>
            <li class="breadcrumb-item active">Voucher Deposit</li>
        </ol>
    </nav>
</div>
        ...
        <div class="card-body py-2">
            <form class="row g-2 align-items-center" method="GET">
                ...
                <div class="col-md-4 d-flex gap-2">
                    ...
                    <a href="voucher-deposit.php" class="btn btn-sm btn-outline-secondary fw-bold">RESET</a>
                </div>
            </form>
        </div>
    </div>
                ...
                <div class="card-body pt-3">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle datatable-premium"
                               data-add-btn='{"text":"New Deposit Voucher","onclick":"openAddTxn()","icon":"fas fa-plus-circle"}'>
                            ...
                            <tbody>
                                ...
                                <tr id="txrow-<?php echo $d['id']; ?>">
                                    <td><div class="fw-bold text-dark"><?php echo date('d M, Y', strtotime($d['date'])); ?></div></td>
                                    <td><code class="text-primary fw-bold">#<?php echo $d['id']; ?></code></td>
                                    <td>
                                        <div class="fw-bold"><?php echo htmlspecialchars($d['head_name']); ?></div>
                                        <small class="text-muted"><?php echo htmlspecialchars($d['description'] ?: ''); ?></small>
                                    </td>
                                    <td><small><?php echo htmlspecialchars($d['transaction_id'] ?: '—'); ?></small></td>
                                    <td><span class="fw-bold text-success">₹ <?php echo number_format($d['amount'], 2); ?></span></td>
                                    <td><span class="badge bg-light text-dark border"><?php echo $d['payment_mode']; ?></span></td>
                                    <td class="text-end">
                                        <button class="btn btn-sm btn-outline-warning btn-icon me-1" title="Edit" onclick='editTxn(<?php echo json_encode($d); ?>)'><i class="fas fa-edit"></i></button>
                                        <button class="btn btn-sm btn-outline-danger btn-icon" title="Delete" onclick="deleteTxn(<?php echo $d['id']; ?>)"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="modalTitle"><i class="fas fa-arrow-down me-2"></i>New Income Deposit</h5>
        ...
        <form id="depositForm" novalidate>
            ...
            <div class="modal-body px-3 py-4">
                <div class="mb-3">
                    <label class="form-label fw-bold">Income Head <span class="text-danger">*</span></label>
                    ...
                    <div class="invalid-feedback">Please select an income head.</div>
                </div>
                <div class="mb-3">
            ...
            <div class="modal-footer bg-light border-top py-2">
                ...
                <button type="submit" class="btn btn-success px-4 fw-bold" id="btn-save">
...
function openAddTxn() {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-arrow-down me-2"></i>New Income Deposit';
...
function editTxn(d) {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit me-2"></i>Edit Deposit Voucher';
    document.getElementById('depositForm').classList.remove('was-validated');
    modalTxn.show();
}
