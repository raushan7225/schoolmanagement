...
$templates = $pdo->query("SELECT id, name FROM document_templates WHERE type = 'marksheet' AND status = 1 ORDER BY name ASC")->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    <table class="table table-hover align-middle datatable-premium">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <button class="btn btn-sm btn-outline-info btn-icon me-1" title="Preview Marksheet" onclick="previewMarksheet(<?php echo $s['id']; ?>)">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button class="btn btn-sm btn-outline-success btn-icon" title="Generate Marksheet" onclick="generateSingle(<?php echo $s['id']; ?>)">
...
document.getElementById('generateSelectedBtn').addEventListener('click', function() {
    ...
    window.open(`print-marksheets.php?ids=${ids.join(',')}&template_id=${tempId}&exam_id=${examId}`, '_blank');
});
...
function previewMarksheet(id) {
    ...
    if (!examId) { alert('Please select an exam.'); return; }
    window.open(`preview-marksheet.php?id=${id}&template_id=${tempId}&exam_id=${examId}`, '_blank');
}
...
function generateSingle(id) {
    ...
    if (!tempId) { alert('Please select a template.'); return; }
    if (!examId) { alert('Please select an exam.'); return; }
    window.open(`print-marksheets.php?ids=${id}&template_id=${tempId}&exam_id=${examId}`, '_blank');
}
</script>
